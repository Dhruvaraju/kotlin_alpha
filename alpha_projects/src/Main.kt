...
public enum class books {
    novel, textbook, notebook, journal
}
...
private fun conditionalOpsWhen() {
    ...
    val book = books.notebook
    val bookType = when (book) {
        books.notebook, books.textbook -> "classwork"
        books.journal, books.novel -> "entertainment"
    }

    println("The current book type is $bookType")
...
private fun exploreArrays() {
    val typicalArray = arrayOf(1, 2, 3, "test")
    for (node in typicalArray) println("$node")

    /* For defining array with a specific type */
    var typeSpecificArray = arrayOf<String>("element001", "element002", "element003")
    for (obj in typeSpecificArray) println("$obj")

    // array with primitive data types
    val anIntArray = intArrayOf(4, 9, 9, 4)
}

private fun alphaTests() {
    val aDoubleVar = 42.3
